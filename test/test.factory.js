...
tape( 'attached to the returned function is the generator name', function test( t ) {
	...
	t.equal( lognormal.NAME, 'lognormal', 'has property' );
	t.end();
});

tape( 'attached to the returned function is the underlying PRNG', function test( t ) {
	var lognormal = factory();
	t.equal( typeof lognormal.PRNG, 'function', 'has property' );

	lognormal = factory({
		'prng': minstd.normalized
	});
	t.equal( lognormal.PRNG, minstd.normalized, 'has property' );
	t.end();
});
...
tape( 'attached to the returned function is the generator seed (integer seed)', function test( t ) {
	...
	t.equal( isUint32Array( lognormal.seed ), true, 'has property' );
	t.equal( lognormal.seed[ 0 ], 12345, 'equal to provided seed' );

	lognormal = factory({
		'seed': 12345,
		'prng': minstd.normalized
	});
	t.equal( lognormal.seed, null, 'equal to `null`' );
	t.end();
});
...
tape( 'attached to the returned function is the generator seed length', function test( t ) {
	var lognormal = factory();
	t.equal( typeof lognormal.seedLength, 'number', 'has property' );

	lognormal = factory({
		'prng': minstd.normalized
	});
	t.equal( lognormal.seedLength, null, 'equal to `null`' );
	t.end();
});

tape( 'attached to the returned function is the generator state', function test( t ) {
	var lognormal = factory();
	t.equal( isUint32Array( lognormal.state ), true, 'has property' );

	lognormal = factory({
		'prng': minstd.normalized
	});
	t.equal( lognormal.state, null, 'equal to `null`' );
	t.end();
});

tape( 'attached to the returned function is the generator state length', function test( t ) {
	var lognormal = factory();
	t.equal( typeof lognormal.stateLength, 'number', 'has property' );

	lognormal = factory({
		'prng': minstd.normalized
	});
	t.equal( lognormal.stateLength, null, 'equal to `null`' );
	t.end();
});

tape( 'attached to the returned function is the generator state size', function test( t ) {
	var lognormal = factory();
	t.equal( typeof lognormal.byteLength, 'number', 'has property' );

	lognormal = factory({
		'prng': minstd.normalized
	});
	t.equal( lognormal.byteLength, null, 'equal to `null`' );
	t.end();
});
...
tape( 'attached to the returned function is a method to serialize the generator as a JSON object', function test( t ) {
	...
	lognormal = factory();
	t.equal( typeof lognormal.toJSON, 'function', 'has method' );

	o = lognormal.toJSON();
	t.equal( o.type, 'PRNG', 'has property' );
	t.equal( o.name, lognormal.NAME, 'has property' );
	...
	t.equal( typeof lognormal.toJSON, 'function', 'has method' );
	t.equal( lognormal.toJSON(), null, 'returns expected value' );
...
tape( 'the function supports specifying the generator state', function test( t ) {
	...
	for ( i = 0; i < 100; i++ ) {
		t.equal( lognormal(), arr[ i ], 'returns expected value. i: '+i+'.' );
	}
	t.end();
});
