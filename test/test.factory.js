...
tape( 'the function throws an error if provided a location parameter which is not a number', function test( t ) {
	var values;
	var i;
...
tape( 'the function returns a seeded pseudorandom number generator (integer seed)', function test( t ) {
	...
	lognormal1 = factory( 0.0, 1.0, {
		'seed': seed
	});
	lognormal2 = factory( 0.0, 1.0, {
		'seed': seed
	});

	t.notEqual( lognormal1, lognormal2, 'separate generators' );
...
tape( 'attached to the returned function is the generator name', function test( t ) {
	...
	t.strictEqual( lognormal.NAME, 'lognormal', 'has property' );
	t.end();
});

tape( 'attached to the returned function is the underlying PRNG', function test( t ) {
	var lognormal = factory();
	t.strictEqual( typeof lognormal.PRNG, 'function', 'has property' );

	lognormal = factory({
		'prng': minstd.normalized
	});
	t.strictEqual( lognormal.PRNG, minstd.normalized, 'has property' );
	t.end();
});
...
tape( 'attached to the returned function is the generator seed (integer seed)', function test( t ) {
	...
	t.strictEqual( isUint32Array( lognormal.seed ), true, 'has property' );
	t.strictEqual( lognormal.seed[ 0 ], 12345, 'equal to provided seed' );

	lognormal = factory({
		'seed': 12345,
		'prng': minstd.normalized
	});
	t.strictEqual( lognormal.seed, null, 'equal to `null`' );
	t.end();
});
...
tape( 'attached to the returned function is the generator seed length', function test( t ) {
	var lognormal = factory();
	t.strictEqual( typeof lognormal.seedLength, 'number', 'has property' );

	lognormal = factory({
		'prng': minstd.normalized
	});
	t.strictEqual( lognormal.seedLength, null, 'equal to `null`' );
	t.end();
});

tape( 'attached to the returned function is the generator state', function test( t ) {
	var lognormal = factory();
	t.strictEqual( isUint32Array( lognormal.state ), true, 'has property' );

	lognormal = factory({
		'prng': minstd.normalized
	});
	t.strictEqual( lognormal.state, null, 'equal to `null`' );
	t.end();
});

tape( 'attached to the returned function is the generator state length', function test( t ) {
	var lognormal = factory();
	t.strictEqual( typeof lognormal.stateLength, 'number', 'has property' );

	lognormal = factory({
		'prng': minstd.normalized
	});
	t.strictEqual( lognormal.stateLength, null, 'equal to `null`' );
	t.end();
});

tape( 'attached to the returned function is the generator state size', function test( t ) {
	var lognormal = factory();
	t.strictEqual( typeof lognormal.byteLength, 'number', 'has property' );

	lognormal = factory({
		'prng': minstd.normalized
	});
	t.strictEqual( lognormal.byteLength, null, 'equal to `null`' );
	t.end();
});
...
tape( 'attached to the returned function is a method to serialize the generator as a JSON object', function test( t ) {
	...
	lognormal = factory();
	t.strictEqual( typeof lognormal.toJSON, 'function', 'has method' );

	o = lognormal.toJSON();
	t.strictEqual( o.type, 'PRNG', 'has property' );
	t.strictEqual( o.name, lognormal.NAME, 'has property' );
	t.deepEqual( o.state, typedarray2json( lognormal.state ), 'has property' );
	t.deepEqual( o.params, [], 'has property' );

	lognormal = factory( 1.0, 2.0 );
	o = lognormal.toJSON();
	...
	t.strictEqual( typeof lognormal.toJSON, 'function', 'has method' );
	t.strictEqual( lognormal.toJSON(), null, 'returns expected value' );
...
tape( 'the function supports providing a seeded underlying PRNG (parameters)', function test( t ) {
	...
	for ( i = 0; i < 1e2; i++ ) {
		r1 = lognormal1( 1.0, 2.0 );
		r2 = lognormal2( 1.0, 2.0 );
		t.strictEqual( r1, r2, 'both return same number' );
	}
	t.end();
});

tape( 'the function returns a PRNG for generating pseudorandom numbers from a lognormal distribution', opts, function test( t ) {
	var threshold;
	var count;
	var npass;
	var sigma;
	...
	function gof() {
		...
		rejected = 0;
		for ( i = 0; i < N; i++ ) {
			lognormal = factory( mu, sigma );
			t.ok( true, 'seed: '+lognormal.seed );
			for ( j = 0; j < x.length; j++ ) {
				x[ j ] = lognormal();
			}
			// Test using Kolmogorov-Smirnov goodness-of-fit test:
			pValue = kstest( x, 'lognormal', mu, sigma ).pValue;
	...
	function done( bool, rejected ) {
		t.ok( bool, 'null hypothesis (i.e., that numbers are drawn from Lognormal('+mu+','+sigma+')) is rejected in less than '+(threshold*100)+'% of cases ('+rejected+' of '+N+'). Repeats: '+npass+' of '+count+'.' );
		t.end();
	}
});
...
tape( 'the function supports specifying the generator state', function test( t ) {
	...
	for ( i = 0; i < 100; i++ ) {
		t.strictEqual( lognormal(), arr[ i ], 'returns expected value. i: '+i+'.' );
	}
	t.end();
});
